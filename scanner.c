...
bool _token_is(TokenType token_type, ...){ // similar to in python (token in (INT,STRING....))
    va_list args;
...
inline void print_token(Token token){
    printf("Token(%s, %d, \"%s\")\n", token_to_str(token.type), token.line,
           token.lexeme);
}

static inline bool is_at_end(ScannerState *ss){ return ss->current >= ss->source_len; }

static inline char advance(ScannerState *ss){ return ss->source[ss->current++]; }
...
static Token hex_number(ScannerState *ss){
    ...
    if(strlen(lexeme) == 2){ // lexeme only contains only 0x 
        exit(-1);
    }
    Token token;
